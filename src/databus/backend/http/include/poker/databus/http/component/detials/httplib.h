...
#    ifdef _WIN32
#        define CPPHTTPLIB_IDLE_INTERVAL_USECOND 10'000
#    else
...
#    define CPPHTTPLIB_REQUEST_URI_MAX_LENGTH 8'192
#endif

#ifndef CPPHTTPLIB_HEADER_MAX_LENGTH
#    define CPPHTTPLIB_HEADER_MAX_LENGTH 8'192
#endif
...
#    define CPPHTTPLIB_MULTIPART_FORM_DATA_FILE_MAX_COUNT 1'024
#endif
...
#    define CPPHTTPLIB_FORM_URL_ENCODED_PAYLOAD_MAX_LENGTH 8'192
#endif

#ifndef CPPHTTPLIB_RANGE_MAX_COUNT
#    define CPPHTTPLIB_RANGE_MAX_COUNT 1'024
#endif
...
#    define CPPHTTPLIB_RECV_BUFSIZ size_t(4'096u)
#endif

#ifndef CPPHTTPLIB_COMPRESSION_BUFSIZ
#    define CPPHTTPLIB_COMPRESSION_BUFSIZ size_t(16'384u)
#endif
...
#    if defined(_MSC_VER)
#        if _MSC_VER < 1'900
#            error Sorry, Visual Studio versions prior to 2'015 are not supported
#        endif
...
#    if OPENSSL_VERSION_NUMBER < 0x30'00'00'00L
...
namespace httplib
{
    ...
    private:
        ...
        {
        private:
            using HandlersType = std::vector< std::pair< std::unique_ptr< detail::MatcherBase >, Handler > >;

        public:
            template < class... Args >
            void emplace_back(Args &&...args)
            {
                std::lock_guard writer_keeper(_keeper);

                _handlers.emplace_back(std::forward< Args >(args)...);
            }

            HandlersType::const_iterator begin() const
            {
                return _handlers.begin();
            }

            HandlersType::const_iterator end() const
            {
                return _handlers.end();
            }

            std::shared_mutex &locker() const
            {
                return _keeper;
            }

        private:
            mutable std::shared_mutex _keeper;
    ...
    namespace detail
    {
        ...
        inline void duration_to_sec_and_usec(const T &duration, U callback)
        {
            ...
            auto usec = std::chrono::duration_cast< std::chrono::microseconds >(duration - std::chrono::seconds(sec))
                                .count();
...
#    endif
    ...
    inline void ClientImpl::set_connection_timeout(const std::chrono::duration< Rep, Period > &duration)
    {
        detail::duration_to_sec_and_usec(duration,
                                         [ & ](time_t sec, time_t usec) { set_connection_timeout(sec, usec); });
    }
    ...
    namespace detail
    {
        ...
        inline size_t to_utf8(int code, char *buff)
        {
            ...
            else if (code < 0x08'00)
            {
                buff[ 0 ] = static_cast< char >(0xC0 | ((code >> 6) & 0x1F));
                buff[ 1 ] = static_cast< char >(0x80 | (code & 0x3F));
                return 2;
            }
            else if (code < 0xD8'00)
            {
            ...
            else if (code < 0xE0'00)
            {   // D800 - DFFF is invalid...
                return 0;
            }
            else if (code < 0x1'00'00)
            {
                buff[ 0 ] = static_cast< char >(0xE0 | ((code >> 12) & 0xF));
                buff[ 1 ] = static_cast< char >(0x80 | ((code >> 6) & 0x3F));
                buff[ 2 ] = static_cast< char >(0x80 | (code & 0x3F));
        ...
        inline ssize_t select_read(socket_t sock, time_t sec, time_t usec)
        {
            ...
            auto timeout = static_cast< int >(sec * 1'000 + usec / 1'000);
...
#    endif
        ...
        private:
            ...
            static const size_t read_buff_size_ = 1'024l * 4;
        };
        ...
        inline bool keep_alive(socket_t sock, time_t keep_alive_timeout_sec)
        {
            ...
            while (true)
            {
                auto val = select_read(sock, 0, 10'000);
                if (val < 0)
                {
                    return false;
                }
                else if (val == 0)
                {
                    ...
                    auto timeout  = keep_alive_timeout_sec * 1'000;
...
#ifdef _WIN32
                            auto timeout =
                                    static_cast< uint32_t >(read_timeout_sec * 1'000 + read_timeout_usec / 1'000);
...
#ifdef _WIN32
                            auto timeout =
                                    static_cast< uint32_t >(write_timeout_sec * 1'000 + write_timeout_usec / 1'000);
...
#    endif
        ...
        bool process_multipart_ranges_data(const Request &req, const std::string &boundary,
                                           const std::string &content_type, size_t content_length, SToken stoken,
                                                       ...
                                                       const std::string &content_type, size_t content_length)
        {
            ...
            process_multipart_ranges_data(
                    req, boundary, content_type, content_length, [ & ](const std::string &token)
                    { data_length += token.size(); }, [ & ](const std::string &token) { data_length += token.size(); },
                    [ & ](size_t /*offset*/, size_t length)
                    {
                        data_length += length;
                        return true;
                                                ...
                                                size_t content_length, const T &is_shutting_down)
        {
            return process_multipart_ranges_data(
                    ...
                    [ & ](const std::string &token) { strm.write(token); }, [ & ](size_t offset, size_t length)
...
#        if TARGET_OS_OSX
        ...
        inline bool retrieve_certs_from_keychain(CFObjectPtr< CFArrayRef > &certs)
        {
            ...
            CFObjectPtr< CFDictionaryRef > query(CFDictionaryCreate(nullptr, reinterpret_cast< const void ** >(keys),
...
#    endif       // _WIN32
        ...
        public:
            ...
            {
                WSADATA wsaData;
                if (WSAStartup(0x00'02, &wsaData) == 0)
                    is_valid_ = true;
            }
                ...
                if (pos != std::string::npos)
                {
                    ...
                    else if (type == "Digest")
                    {
                        ...
                        for (auto i = beg; i != std::sregex_iterator(); ++i)
                        {
                            const auto &m = *i;
                            auto        key =
                                    s.substr(static_cast< size_t >(m.position(1)), static_cast< size_t >(m.length(1)));
    ...
    namespace detail
    {
        ...
        {
#if defined(_MSC_VER) && _MSC_VER < 1'910
    ...
    inline bool Server::listen_internal()
    {
...

        {
            ...
            while (svr_sock_ != INVALID_SOCKET)
            {
                ...
                {
#ifdef _WIN32
                    auto timeout = static_cast< uint32_t >(read_timeout_sec_ * 1'000 + read_timeout_usec_ / 1'000);
...
#ifdef _WIN32
                    auto timeout = static_cast< uint32_t >(write_timeout_sec_ * 1'000 + write_timeout_usec_ / 1'000);
    ...
    {
        {
            std::shared_lock< std::shared_mutex > traverse_keeper(handlers.locker());
